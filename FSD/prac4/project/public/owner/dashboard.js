...
function renderStatistics() {
    ...
    const available = properties.filter(p => p.availability === "available").length;
...
function getFilteredSorted() {
    ...
    switch (currentSort) {
        case "oldest":
            ...
            break;
        case "rent-low":
            list.sort((a, b) => (Number(a.monthlyRent) || 0) - (Number(b.monthlyRent) || 0));
            break;
        case "rent-high":
            list.sort((a, b) => (Number(b.monthlyRent) || 0) - (Number(a.monthlyRent) || 0));
            break;
        default: // newest
...
function renderProperties() {
    ...
    dom.propertiesGrid.innerHTML = pageItems.map(propertyCardHTML).join("");
...
function renderAltGrid(items) {
    ...
    dom.propertiesGridAlt.innerHTML = items.length
        ? items.map(propertyCardHTML).join("")
        : `<div class="col-12"><div class="empty-state text-center py-5">
             <h5 class="mb-2">No properties to show</h5>
             <p class="text-muted mb-0">Add your first property to see it here.</p>
           </div></div>`;
}
...
function switchPage(pageName) {
    ...
    if (propertiesPage) propertiesPage.classList.toggle("active", pageName === "properties");
...
function statusLabel(status) {
    return { available: "Available", occupied: "Occupied", pending: "Pending" }[status] || "Unknown";
}

function propertyTypeLabel(type) {
    return { pg: "PG", hostel: "Hostel", apartment: "Apartment" }[type] || type || "Property";
}
...
function filterLabel(filter) {
    return { all: "All", available: "Available", occupied: "Occupied", pending: "Pending" }[filter] || "All";
}
...
dom.currentFilterLabel.closest(".dropdown").parentElement.addEventListener("click", (e) => {
...
dom.propertiesGrid.addEventListener("click", (e) => {
    ...
    if (deleteBtn) {
        ...
        showConfirmation(deleteBtn.dataset.id, property ? property.propertyName : "this property");
    }
});
